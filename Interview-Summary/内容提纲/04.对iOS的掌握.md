...
4. [block原理](https://gsl201600.github.io/2020/05/13/iOSblock%E5%8E%9F%E7%90%86%E8%AF%A6%E8%A7%A3/)
   ...
   * Block具有捕获外部变量的特性
     ...
     自动变量不加`__block`时不允许在Block中修改捕获的自动变量（不包括有强指针引用或copy修饰的成员属性引用的block），但允许修改其它几种变量。

     未加`block`修饰符的非对象类型自动变量以值传递方式传递到Block的构造函数里面。由于只捕获了自动变量的值，而不是内存地址，所以Block内部不能改变自动变量的值。而静态变量和对象类型的自动变量传递的是地址，所以可以改变值。

     总结一下在Block中改变的两种方式，一是传递内存地址指针到Block中，二是改变存储方式（`__block`修饰符）。对于全局变量和函数参数，由于作用域足够大，所以Block不用捕获这两种变量到Block中持有，可直接修改。
   ...
   * Block中__block变量的实现原理
     ...
     **对于对象变量**
     ...
     而在MRC环境下，这个时候Block在栈上，NSStackBlock，`__block`不会对指针指向的对象执行copy操作，而只是把指针进行复制（不持有对象）。当手动把Block copy一下，就变成了NSMallocBlock，并持有对象，引用计数加一。
     ...
     * 静态变量和带`__block`的自动变量，都是直接地址访问，可以在Block里面改变变量的值
