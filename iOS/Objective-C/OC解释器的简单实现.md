...
本篇文章的内容基于**《跟戴铭学iOS编程》**一书中的OC解释器实现部分。简单阐述了**解释器的工作原理**和**OC解释器的简单实现**。希望通过对这一部分知识的整理和思考，能对解释器有更多的了解。
...
一个解释器可大可小，大可以是复杂的CPU，小也可以是一个简单的字符串解析，但本质上它们都是**对特定的语法做出合理的解释。**
...
在这个过程中，编译器调用预处理器，**将源代码进行预处理**（清除注释、翻译宏定义、包含文件和条件编译）。通过把经过**预处理的源代码编译成目标代码**（二进制机器语言），再**调用链接器外加库文件，从而形成可执行程序**，让机器能够执行。
...
编译器和解释器各有优缺点，随着技术的发展，一些技术的出现也在慢慢模糊他们的边界。例如有些解释器并不像名称所说的那样运行原始代码，而是把原始代码转换成**更压缩的格式**（字节码、抽象语法树等），有些解释器还会以**解释器包含的编译器**对高级语言编译，并指示处理器运行编译后的程序。但不管怎样，编译器和解释器还是有着区分的，下面是他们的一些比较：

##### # 运行时机的不同

对于编译器，**程序的编译和执行是独立的两个阶段**，编译器先把项目源码编译生成一个独立的可执行程序，再执行这个可执行程序。而解释的程序总是需要解释器来执行，在运行时解释器逐句代码解释成机器指令并执行（或者把中间代码解释执行），**源代码的解释和执行是同一阶段**。

##### # 执行效率的不同

因为编译器是先编译后执行，编译得到的可执行程序对应的是可直接执行的机器码，所以**运行时无需再依赖编译器或额外的程序**，运行速度较快。

而使用解释型语言编写的项目，每次运行程序时，解释器都会先把程序源码/中间产物转成另一种语言再作运行，因此解释器的程序运行速度比较缓慢。**它不会一次性翻译整个程序**，而是每翻译一行程序叙述就立刻运行，然后再去翻译下一行，如此不停地进行下去。

可以看到解释器的好处是**解释器消除了编译整个程序的负担**（无需提前编译整个项目，每翻译一句语句执行一句），程序可以拆分成多个部分来**模块化**。但这会让运行时的效率打折扣，而编译器已一次性将所有源代码翻译成机器码（可执行程序），可以直接执行，所以**解释型语言的执行效率会比编译型语言的执行效率低。**
...
* 直接解释运行：直接运行高级编程语言
* 生成字节码：转换高级语言
* 编译后程序：以解释器包含的编译器对高级语言编译，并指示处理器运行编译后的程序（如JIT）

#### 直接运行高级编程语言

直接把高级编程语言的源代码转译成机器码执行。
...
第一次翻译时，把源代码完整地进行转换，编译成更高效的字节码。如果后续源代码文件没有修改，就直接将对应字节码直接翻译成机器码执行，从而提高速度。这个转换出来的的中间字节码也可以被说成是虚拟机的机器代码，然后被虚拟机解释和运行。

#### JIT即时编译

JIT进一步模糊了解释器，字节码解释器和编译之间的区别，该技术是在运行时将中间表示形式编译为原生的机器码。这提高了运行原生码的效率，但在第一次编译时增加了启动时间的开销和内存使用。和适应性优化是互补的技术，在适应性优化的支持下，解释器分析运行中的程序并把其中执行最频繁的部分编译成机器指令。执行频繁的内容包括多次调用的方法体和多次执行的循环体。

m记: 畏寒（手脚冰冷）、易累（胸、小腹和腰部疼痛），情绪波动，周期一般是28天（不确定），好好照顾L（抱抱、准备热饮、不让L提太重的东西、不能让L吃冷的）

## OC解释器的设计与实现

用低级语言编写的简单解释器可能具有实现高级语言功能的类似机器代码块，并在查找表中的函数条目指向该代码时执行。但是以高级语言编写的解释器通常使用另一种方法，例如生成然后遍历解析树，或者生成并执行中间软件定义的指令，或者同时使用这两种方法。
...
部分观点认为，当程序运行环境中**内存资源限制较大**，可以使用解释器执行节约内存，反之可以使用编译执行来提升效率。这里有一个问题，嵌入式设备应该是符合内存资源限制较大这一场景的，但嵌入式通常是采用C/C++这类编译型的编程语言？
...
* [解释器——维基百科](https://en.wikipedia.org/wiki/Interpreter_(computing))
