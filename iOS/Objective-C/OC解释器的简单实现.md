...
编译器和解释器各有优缺点，随着技术的发展，下面是他们的一些比较：

* 运行时机的不同

  解释器立即执行程序，在运行时解释器逐句代码解释成机器指令并执行（或者把中间代码解释执行），编译器为稍后的执行准备好可执行程序。

  即对于编译器，**程序的编译和执行是独立的两个阶段**，编译器先把项目源码编译生成一个独立的可执行程序，再执行这个可执行程序。而解释的程序总是需要解释器来执行，**源代码的解释和执行是同一阶段**。

* 执行效率的不同

  使用编译型语言编写的项目在经过编译后，得到可执行程序，可执行程序对应的是可直接执行的机器码，所以**运行时无需再依赖编译器或额外的程序**。

  而使用解释型语言编写的项目，每次运行程序时，解释器都会先把程序源码/中间产物转成另一种语言再作运行，因此解释器的程序运行速度比较缓慢。**它不会一次性翻译整个程序**，而是每翻译一行程序叙述就立刻运行，然后再去翻译下一行，如此不停地进行下去。

  可以看到解释器的好处是**解释器消除了编译整个程序的负担**（无需提前编译整个项目，每翻译一句语句执行一句），程序可以拆分成多个部分来**模块化**。但这会让运行时的效率打折扣，而编译器已一次性将所有源代码翻译成机器码，可以直接执行，所以**解释型语言的执行效率会比编译型语言的执行效率低。**
