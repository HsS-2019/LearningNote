...
* 转换成中间表示，然后立刻执行：转换高级语言

  包括生成抽象语法树、字节码等方式

* 显式执行存储的由作为解释器系统的一部分的编译器生成的预编译代码。

* JIT即时编译：以解释器包含的编译器对高级语言编译，并指示处理器运行编译后的程序
...
JIT进一步模糊了解释器，字节码解释器和编译之间的区别，该技术是在运行时将中间表示形式编译为原生的机器码。这提高了运行原生码的效率，但在第一次编译时增加了启动时间的开销和内存使用。和适应性优化是互补的技术，在适应性优化的支持下，解释器分析运行中的程序并把其中执行最频繁的部分编译成机器指令。

畏寒（手脚冰冷）、易累（胸、小腹和腰部疼痛），情绪波动，周期一般是28天（不确定），好好照顾L（抱抱、准备热饮、不让L提太重的东西、不能让L吃冷的）
